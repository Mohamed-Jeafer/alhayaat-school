...
Rewrote `src/app/admissions/page.tsx` with Webflow-aligned layout. Sections: admission banner image, enrollment cards (orange/yellow `ColoredBorderCard`), Why Choose cards (3-col grid, blue accent), How to Apply (numbered steps), Required Documents (downloadable forms grid + documents checklist), fee schedule table, and CTA. All content from `admissions.json`.
